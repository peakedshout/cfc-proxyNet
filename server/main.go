...
func checkFile() {
	var err error
	_, err = os.Stat("./asset/cfc-proxyNetS/main.go")
	errCheck(err)
	...
	for key, value := range goGoArchMap {
		for _, one := range value {
			err = os.MkdirAll("./asset/complete_resources/server/"+key+"_"+one, 0777)
			errCheck(err)
		}
	}
}
...
func makeConfig() {
	...
	fn2 := func(key string) string {
		return "./asset/complete_resources/server/config" + key
	}
	for key, value := range configMMap {
		...
		errCheck(err)
		err = os.WriteFile(fn2(key)+"/config.json", b, 0666)
		errCheck(err)
	}
}
...
func makeBin(goos, arch string) {
	...
	cmd = exec.Command("go", "build", "-ldflags", "-s -w", "-o", s, "./asset/cfc-proxyNetS")
...
func getOutPath(goos, arch string) string {
	out := "./asset/complete_resources/server/" + goos + "_" + arch + "/cfc-proxyNetS_" + goos + "_" + arch
